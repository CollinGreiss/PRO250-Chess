...
mainAI::mainAI(Piece::Team playerTeam, std::vector<Piece*> pieces) {
	...
	for (Piece* piece : pieces) {
		...
		int row = piece->getPos().yCoord + 2;
		int col = piece->getPos().xCoord + 2;
		char type = piece->getType();

		if (piece->getTeam() == Piece::Team::WHITE) type = toupper(type);
		else type = tolower(type);

		root->board[row][col] = type;
